...
if __name__ == '__main__':
    ...
    grid_tiles = [1831, 3607, 2677, 3449, 1451, 1013, 2953, 2999, 2731, 3583, 3529, 1699, 1423, 3671, 3461, 2971, 3023,
                  1249, 2687,
                  3539, 3313, 2593, 3329, 2251, 1069, 2179, 2903, 2621, 2549, 2083, 3209, 1697, 1657, 2963, 1499, 1367,
                  1877, 1973,
                  3001, 3673, 2897, 3221, 2081, 1693, 2927, 2437, 2939, 3767, 1871, 3467, 2887, 1433, 3343, 3697, 3413,
                  1511, 2129,
                  1951, 2339, 2693, 3989, 2423, 3931, 1453, 2917, 1949, 3331, 3433, 3911, 1307, 2267, 3929, 3851, 2053,
                  3217, 3797,
                  2089, 3259, 1759, 2341, 3011, 1153, 1123, 2441, 2087, 3517, 2297, 1087, 2221, 1481, 1459, 1471, 2237,
                  2281, 2357,
                  2017, 1747, 2069, 3041, 3769, 3643, 2459, 2467, 1103, 1279, 2203, 1847, 2011, 2663, 1171, 1303, 3631,
                  2311, 3079,
                  3533, 3919, 1151, 2389, 1789, 1667, 2633, 1597, 1487, 3547, 3943, 3581, 1039, 1907, 1109, 2671, 3557,
                  1559, 2789,
                  1549, 2381, 3617, 2729, 2647, 2797, 2417, 2029, 2767, 3391, 2309]
    grid_orientations = [
        2, 0, 3, 5, 4, 0, 2, 5, 5, 0, 0, 3, 3, 0, 6, 2, 6, 0, 3, 6, 6, 5, 3, 5, 4, 5, 5, 2, 5, 3, 5, 2, 4, 5, 5, 6, 0,
        4,
        4, 0, 6, 2, 5, 0, 3, 5, 0, 1, 3, 4, 2, 6, 0, 3, 2, 5, 5, 1, 5, 0, 6, 5, 5, 2, 6, 1, 3, 6, 3, 5, 2, 3, 3, 2, 6,
        6,
        0, 0, 3, 0, 3, 5, 2, 1, 3, 6, 6, 0, 1, 1, 2, 3, 2, 5, 4, 2, 4, 6, 3, 6, 6, 2, 4, 4, 2, 5, 4, 1, 5, 5, 1, 2, 3,
        4,
        4, 2, 1, 5, 0, 1, 0, 3, 6, 2, 6, 2, 0, 5, 5, 4, 3, 0, 6, 3, 3, 5, 5, 0, 5, 5, 1, 3, 2, 3]
    #print(solve_grid(0))
    print(grid_tiles)
    ...
    for tile_num in range(len(tiles)):
        for y in range(tile_len - 2):
            for x in range(tile_len - 2):
                n = tiles[grid_tiles[tile_num]][1][1:tile_len - 2]
                e = ''.join([tiles[tile_num][x][tile_len - 1] for x in range(tile_len)])
                s = tiles[tile_num][tile_len - 1]
                w = ''.join([tiles[tile_num][x][0] for x in range(tile_len)])
